...
public class TimedAnimationController
{
    ...
    public TimedAnimationController(Animator animator, string layerName = "Base Layer")
    {
            ...
            else if (_layerIndex > 0)
            {
                ...
                if (Math.Abs(weight) < Mathf.Epsilon)
                {
                    Debug.LogWarningFormat(
                        $"Layer `{layerName}`:{_layerIndex} of {_animator.name} has zero weight, setting to 1.0.",
                        _animator);
        ...
        if (!_animator.HasState(_layerIndex, _clipNameHash))
        {
            ...
            Debug.LogAssertion(
                $"Could not find State {clipName} ClipDefinition in Layer {layerName} TimedAnimationController. " +
                $"This usually happens when renaming animation clips, ensure to rename the animator state as well.",
                animator);
            ...
            Debug.LogWarningFormat($"State `{clipName}` was not found, using replacementState {replacementState}.",
                animator);
        }
    }
